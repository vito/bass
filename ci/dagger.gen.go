...
import (
	...
	"github.com/vito/bass/querybuilder"
)
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Bass":
		...
		case "":
			...
			var code *Directory
			if inputArgs["code"] != nil {
				err = json.Unmarshal([]byte(inputArgs["code"]), &code)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg code", err))
				}
			}
			return New(code), nil
		default:
			...
			WithObject(
				dag.TypeDef().WithObject("Bass").
					...
					WithFunction(
						dag.Function("Base",
							dag.TypeDef().WithObject("Container"))).
					WithField("Code", dag.TypeDef().WithObject("Directory")).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("Bass")).
							WithArg("code", dag.TypeDef().WithObject("Directory")))), nil
	default:

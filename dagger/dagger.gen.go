...
func LoadHomeFromID(r *Client, id HomeID) Home {
	q := r.Query.Select("loadBassHomeFromID")
	q = q.Arg("id", id)
	return &homeImpl{
		Client: r.Client,
		Query:  q,
...
func (r *homeImpl) XXX_GraphQLType() string {
	return "Home"
}

func (r *homeImpl) XXX_GraphQLIDType() string {
	return "HomeID"
}

func (r *homeImpl) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
...
func (r *homeImpl) MarshalJSON() ([]byte, error) {
	if r == nil {
		return []byte("\"\""), nil
	}
...
func (r *homeImpl) UnmarshalJSON(bs []byte) error {
	var id HomeID
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = *LoadHomeFromID(dag, id).(*homeImpl)
	return nil
}

func (r *homeImpl) toIface() Home {
	if r == nil {
		return nil
	}
	return r
...
func (r *homeImpl) ID(ctx context.Context) (HomeID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.Query.Select("id")
...
func (r *homeImpl) Install(base *Container) *Container {
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Bass":
		...
		case "Build":
			...
			return (*Bass).Build(&parent, version), nil
		case "Repl":
			var parent Bass
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		case "Unit":
			...
			return (*Bass).Unit(&parent, packages), nil
		case "RunUnit":
			var parent Bass
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Bass).RunUnit(&parent, ctx)
		case "Buildkitd":
			var parent Bass
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Bass).Buildkitd(&parent), nil
		case "DevContainer":
			var parent Bass
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			var home *homeImpl
			if inputArgs["home"] != nil {
				err = json.Unmarshal([]byte(inputArgs["home"]), &home)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg home", err))
				}
			}
			return (*Bass).DevContainer(&parent, home.toIface()), nil
		case "Dev":
			var parent Bass
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		case "Generate":
			...
			return (*Bass).Generate(&parent), nil
		case "Code":
			var parent Bass
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				...
				dag.TypeDef().WithObject("Bass").
					WithFunction(
						dag.Function("Build",
							...
							WithArg("version", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"dev\"")})).
					WithFunction(
						dag.Function("Repl",
					...
					WithFunction(
						dag.Function("Unit",
							...
							WithArg("packages", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)).WithOptional(true))).
					WithFunction(
						dag.Function("RunUnit",
							dag.TypeDef().WithKind(StringKind))).
					WithFunction(
						dag.Function("Buildkitd",
							dag.TypeDef().WithObject("Service"))).
					WithFunction(
						dag.Function("DevContainer",
							dag.TypeDef().WithObject("Container")).
							WithArg("home", dag.TypeDef().WithInterface("Home"))).
					WithFunction(
						dag.Function("Dev",
							dag.TypeDef().WithObject("Terminal"))).
					WithFunction(
					...
					WithFunction(
						dag.Function("Base",
							dag.TypeDef().WithObject("Container"))).
					WithField("Src", dag.TypeDef().WithObject("Directory")).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("Bass")).
							WithArg("src", dag.TypeDef().WithObject("Directory")))).
			WithInterface(
				dag.TypeDef().WithInterface("Home").
					WithFunction(
						dag.Function("Install",
							dag.TypeDef().WithObject("Container")).
							WithArg("base", dag.TypeDef().WithObject("Container")))), nil
	default:
